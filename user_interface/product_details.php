...
$current_page_user_id = $user_is_logged_in ? (int)$_SESSION['user_id'] : 0;
$display_name = $user_is_logged_in ? htmlspecialchars($_SESSION['user_name'] ?? 'User') : "Guest";
...
<body class="antialiased">
    ...
    <header class="header-bg sticky top-0 z-50 shadow-2xl">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-20">
            <a href="index_user.php<?php echo $user_is_logged_in ? '?user_id='.$current_page_user_id : ''; ?>" class="text-3xl md:text-4xl font-orbitron font-bold tracking-wider" aria-label="MuscleMafia Homepage">
                Muscle<span class="neon-accent-text">Mafia</span>
            </a>
            <nav class="hidden md:flex items-center space-x-1">
                <a href="index_user.php<?php echo $user_is_logged_in ? '?user_id='.$current_page_user_id.'#showAllProductsBtn' : '#showAllProductsBtn'; ?>" class="nav-link px-3 py-2">Categories</a>
                <a href="index_user.php<?php echo $user_is_logged_in ? '?user_id='.$current_page_user_id.'#allProductsTitle' : '#allProductsTitle'; ?>" class="nav-link px-3 py-2">Showcase</a>
         ...
         <div id="mobileMenu" class="md:hidden hidden bg-black bg-opacity-95 absolute w-full shadow-xl border-t border-gray-800">
            <a href="index_user.php<?php echo $user_is_logged_in ? '?user_id='.$current_page_user_id.'#showAllProductsBtn' : '#showAllProductsBtn'; ?>" class="block nav-link text-center py-3 border-b border-gray-800 mobile-menu-link">Categories</a>
            <a href="index_user.php<?php echo $user_is_logged_in ? '?user_id='.$current_page_user_id.'#allProductsTitle' : '#allProductsTitle'; ?>" class="block nav-link text-center py-3 border-b border-gray-800 mobile-menu-link">Showcase</a>
            <?php if ($user_is_logged_in && $current_page_user_id > 0): ?>
                ...
                <a href="<?php echo $login_page_url; ?>?logout=true" class="block nav-link nav-link-cta text-center py-3 mx-4 my-3 mobile-menu-link">Logout</a>
            <?php else: ?>
    ...
    <main class="container mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <?php if ($product): ?>
            <div class="mb-8">
                 <a href="index_user.php<?php echo $user_is_logged_in ? '?user_id='.$current_page_user_id : ''; ?>" class="return-button">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 md:gap-12 items-start">
                <div class="lg:col-span-3 product-detail-card rounded-xl p-4 md:p-6 scroll-animate fade-in-up">
                    ...
                    <div class="mt-4 text-center text-sm text-gray-400">
                        <p>Hover/Click image for zoom (Feature coming soon)</p>
                    </div>
                </div>

                <div class="lg:col-span-2 product-detail-card rounded-xl p-6 md:p-8 scroll-animate fade-in-up" style="animation-delay: 0.15s;">
                    <a href="index_user.php<?php echo $user_is_logged_in ? '?user_id='.$current_page_user_id : ''; ?>#category-<?php echo htmlspecialchars($product['category_id']); ?>" class="text-sm text-cyan-400 hover:text-cyan-300 uppercase tracking-wider font-medium">
        ...
        <?php else: ?>
            <div class="text-center py-20 min-h-[60vh] flex flex-col items-center justify-center">
                ...
                <a href="index_user.php<?php echo $user_is_logged_in ? '?user_id='.$current_page_user_id : ''; ?>" class="return-button text-lg">
    ...
    <footer class="footer-bg text-gray-400 text-center py-12 md:py-16 border-t border-gray-800 mt-16">
        <div class="container mx-auto px-4">
            <a href="index_user.php<?php echo $user_is_logged_in ? '?user_id='.$current_page_user_id : ''; ?>" class="text-3xl font-orbitron font-bold tracking-wider text-white" aria-label="MuscleMafia Homepage">
                Muscle<span class="neon-accent-text">Mafia</span>
            </a>
